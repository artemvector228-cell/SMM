  ...
  return (
    <section id="results" ref={ref} aria-label="Результаты" className="bg-[#FAFAF8] py-20 sm:py-28 lg:py-36">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        ...
        >
          <p className="text-[0.8125rem] font-bold text-[#15803D] uppercase tracking-[0.08em] mb-4">
            Реальные результаты
          </p>
          <h2
          ...
          >
            Конкретные цифры от конкретных ниш
          </h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-16">
          {CASES.map((c, i) => (
            <motion.article
              ...
              className="bg-white border border-[#E5E4E0] rounded-2xl p-6 flex flex-col gap-4 hover:border-[#D4D4CF] hover:shadow-[0_8px_28px_rgba(0,0,0,0.07)] transition-all duration-200"
            >
              {/* Header */}
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-[#8A8A8A] uppercase tracking-wider">{c.niche}</span>
                <span className="text-xs font-semibold text-[#15803D] bg-[#F0FDF4] border border-[#BBF7D0] px-2.5 py-1 rounded-full">
                  {c.goal}
                </span>
              </div>
              ...
              <div className="flex items-end gap-1.5">
                <span
                  className="text-[#141414] leading-none"
                  style={{
                    fontFamily: 'var(--font-space-grotesk)',
                ...
                <div className="flex flex-col mb-1">
                  <span className="text-base font-bold text-[#141414] leading-none">{c.metricUnit}</span>
                  <span className="text-xs text-[#8A8A8A] leading-none mt-0.5">{c.period}</span>
                </div>
              </div>

              <p className="text-sm text-[#525252] leading-[1.7] flex-1">{c.story}</p>
            </motion.article>
          ))}
        </div>
        ...
        <motion.div
          ...
          className="bg-white border border-[#E5E4E0] rounded-2xl p-7 sm:p-9 max-w-xl"
        >
          <div className="flex items-start justify-between mb-7">
            <div>
              <p className="text-[0.8125rem] font-bold text-[#1D4ED8] uppercase tracking-[0.06em] mb-1">Quality Gate</p>
              <h3
                className="text-[#141414]"
                style={{ fontFamily: 'var(--font-space-grotesk)', fontSize: '1.1875rem', fontWeight: 700, letterSpacing: '-0.015em' }}
              >
                Как AI оценивает каждый текст
              </h3>
            </div>
            <div className="flex flex-col items-end">
              <span
                className="text-[#15803D] leading-none"
                style={{ fontFamily: 'var(--font-space-grotesk)', fontSize: '2.25rem', fontWeight: 800, letterSpacing: '-0.04em' }}
              >
                87
              </span>
              <span className="text-xs text-[#8A8A8A]">из 100</span>
            </div>
          </div>
              ...
              return (
                <div key={i}>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-sm text-[#525252] font-medium">{cr.label}</span>
                    <span className="text-sm font-bold text-[#141414]">
                      {cr.score}
                      <span className="text-[#8A8A8A] font-normal">/{cr.max}</span>
                    </span>
                  </div>
                  <div className="h-1.5 bg-[#F4F3F0] rounded-full overflow-hidden">
                    <motion.div
                      ...
                      style={{ background: isGreen ? '#15803D' : '#1D4ED8' }}
                    />
                  </div>
                </div>
              )
          ...
          <p className="text-sm text-[#8A8A8A] mt-5 leading-relaxed">
            Если итог ниже 80 — AI находит конкретные проблемы и переписывает их. Вы получаете только проверенный текст.
          </p>
        </motion.div>
      </div>
    </section>
